...
public:
    ...
    std::unique_ptr<Vector3f[]> vertices;//顶点数量
    uint32_t numTriangles;//三角形数量
    std::unique_ptr<uint32_t[]> vertexIndex;//每个三角形对应的顶点的index, 例如第k个三角形,三个顶点就是vertices[vertexIndex[k]] vertices[vertexIndex[k + 1]] vertices[vertexIndex[k + 2]]
