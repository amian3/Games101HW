...
*/
        ...
        {
            auto beginning = objects.begin();
            auto middle = (objects.size() * i / bucket);
            auto endding = objects.end();
            std::vector<Object*> leftobjects(objects.begin(), objects.begin() + middle);
            std::vector<Object*> rightobjects(objects.begin() + middle, objects.end());
            Bounds3 left, right;
            for(int j = 0;j < leftobjects.size(); ++j)
                left = Union(left, leftobjects[j]->getBounds().Centroid());
            for(int j = 0;j < rightobjects.size(); ++j)
                right = Union(right, rightobjects[j]->getBounds().Centroid());
            float curcost = left.SurfaceArea() * leftobjects.size() + right.SurfaceArea() * rightobjects.size();
            if(curcost < mincost){
                mincost = curcost;
                middleindex = middle;
            }
